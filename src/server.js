...
<div class="nav-links"><span class="badge ${pb}">${user.plan.toUpperCase()}</span><a href="/review-analytics">📊 Analytics</a><a href="/response-templates">📝 Templates</a><a href="/pricing">Upgrade</a><a href="/logout">Log Out</a></div></div></nav>
...
<div class="stat-card" style="border-left:4px solid #f59e0b"><div class="stat-value">${aggAvg ? aggAvg.toFixed(1) + '★' : '—'}</div><div class="stat-label">Avg Rating</div></div>
...
<div class="stat-card" style="border-left:4px solid #22c55e"><div class="stat-value">${aggCaught}</div><div class="stat-label">Bad Reviews Caught 🛡️</div></div>
</div>`;

  // Notifications
  if (notifs.length) {
    html += `<div class="card" style="margin-bottom:20px;border-left:4px solid #f59e0b;padding:16px 20px"><h3 style="font-size:15px;margin-bottom:12px">🔔 Recent Notifications</h3>`;
    notifs.forEach(n => {
...
<span style="font-size:18px">📧</span><span style="flex:1">${esc(n.message)}</span>
...
<a href="/r/${firstLoc.slug}" target="_blank" rel="noopener noreferrer" class="btn btn-secondary" style="text-align:center;padding:16px 12px;font-size:14px">👁️ Preview Review Page</a>
...
<a href="/guides" class="btn btn-secondary" style="text-align:center;padding:16px 12px;font-size:14px">🚀 Get More Reviews</a>
  ...
  if (!locs.length) {
    html += `<div class="card" style="text-align:center;padding:60px 20px"><div style="font-size:64px;margin-bottom:16px">🏪</div>
<h2 style="font-size:24px;margin-bottom:8px">No reviews yet — share your link to get started!</h2>
...
<div><a href="/locations/new" class="btn btn-primary" style="padding:14px 32px;font-size:16px">+ Add Your First Location</a></div>
<div style="margin-top:32px;display:flex;justify-content:center;gap:32px;color:#94a3b8;font-size:13px">
<div>✅ Free forever plan</div><div>✅ 2-minute setup</div><div>✅ No credit card</div></div></div>`;
  } else {
    locs.forEach(l => {
...
<a href="/r/${l.slug}" target="_blank" rel="noopener noreferrer" class="btn btn-secondary btn-sm">Preview</a>
...
${[['blue','🔵 Blue (Default)'],['green','🟢 Green'],['purple','🟣 Purple'],['orange','🟠 Orange']].map(([v,l])=>`<option value="${v}" ${(loc?.color_theme||'blue')===v?'selected':''}>${l}</option>`).join('')}</select></div>
...
app.get('/review-analytics', requireAuth, (req, res) => {
  ...
  if (!locIds.length) {
    return res.send(dashLayout(user, '<div style="text-align:center;padding:60px"><h1>Review Analytics</h1><p style="color:#64748b;margin:16px 0">Add a location first to see analytics.</p><a href="/locations/new" class="btn btn-primary">+ Add Location</a></div>'));
  }
  ...
  const content = '<script src="https://cdn.jsdelivr.net/npm/chart.js@4.4.7/dist/chart.umd.min.js"><\/script>' +
    ...
    '<div><h1 style="font-size:28px">📊 Review Analytics Dashboard</h1>' +
    '<p style="color:#64748b;font-size:14px">Insights across all your locations</p></div>' +
    '<a href="/dashboard" class="btn btn-secondary btn-sm">\u2190 Back to Dashboard</a></div>' +
    ...
    'new Chart(document.getElementById("sentimentChart"),{type:"bar",data:{labels:CD.sentimentLabels,datasets:[{label:"Positive (4-5★)",data:CD.sentimentPos,backgroundColor:"#22c55e"},{label:"Neutral (3★)",data:CD.sentimentNeu,backgroundColor:"#f59e0b"},{label:"Negative (1-2★)",data:CD.sentimentNeg,backgroundColor:"#ef4444"}]},options:{responsive:true,scales:{x:{stacked:true},y:{stacked:true,beginAtZero:true}},plugins:{legend:{position:"bottom"}}}});' +
    'new Chart(document.getElementById("distChart"),{type:"bar",data:{labels:["1★","2★","3★","4★","5★"],datasets:[{data:CD.ratingDist,backgroundColor:["#ef4444","#f97316","#f59e0b","#84cc16","#22c55e"],borderRadius:8}]},options:{indexAxis:"y",responsive:true,plugins:{legend:{display:false}},scales:{x:{beginAtZero:true,ticks:{stepSize:1}}}}});' +
    '});<\/script>';
...
app.post('/r/:slug/submit', (req, res) => {
  ...
  if (!redir) {
    ...
    if (owner) {
      const msg = `⚠️ New ${r}-star review at ${loc.business_name}${customer_name ? ' from '+customer_name : ''} — Email notification sent!`;
...
<label style="font-size:14px;font-weight:600;margin-bottom:8px;display:block">🔍 Search for your business</label>
...
💡 Go to your Google Business Profile → click "Ask for reviews" → copy the link
</div></div>
...
<span style="transition:transform .2s;display:inline-block">▶</span> How to find your Google review link
</summary>
...
<li>A popup will show your review link — <strong>copy it</strong></li>
<li>Paste it in the field above</li>
</ol>
<div style="background:#fef3c7;border-radius:8px;padding:10px 12px;margin-top:8px;font-size:12px;color:#92400e">
⚠️ <strong>Don't have a Google Business Profile?</strong> You can create one free at <span style="font-weight:600">business.google.com</span> — or use the search above to find your business and we'll generate a link for you.
</div>
</div></details>

<button class="btn btn-primary onb-next" onclick="goStep(2)">Continue →</button>
<button type="button" class="btn btn-secondary" onclick="document.getElementById('ob-url').value='';goStep(2)" style="width:100%;margin-top:8px;padding:14px;font-size:15px;background:#f8fafc;border:2px dashed #cbd5e1;color:#64748b">⏭️ I'll add this later — you can always update it from your dashboard</button>
</div>
  ...
  try{
    ...
    if(d.success){
      document.getElementById('ob-final').innerHTML='<div style="font-size:48px;margin-bottom:16px">🎉</div><h3 style="margin-bottom:8px">Your review page is live!</h3><p style="color:#64748b;margin-bottom:20px">Share this link with your customers:</p>'
        +'<div style="background:#f0fdf4;border:2px solid #bbf7d0;border-radius:12px;padding:16px;margin-bottom:20px;word-break:break-all"><a href="'+d.url+'" target="_blank" rel="noopener noreferrer" style="color:#16a34a;font-weight:600;font-size:16px">'+d.url+'</a></div>'
  ...
  try {
    ...
    if (d.results && d.results.length > 0) {
      rd.innerHTML = d.results.map(p =>
        ...
        (p.reviewUrl ? '<div style="font-size:12px;color:#16a34a;margin-top:4px">✓ Google review link available</div>' : '<div style="font-size:12px;color:#f59e0b;margin-top:4px">⚠ Review link may need manual entry</div>') +
        '</div>'
      ).join('');
    } else if (d.fallback) {
      rd.innerHTML = '<div style="padding:12px;background:#fef3c7;border-radius:10px;font-size:13px;color:#92400e">' +
        ...
        '<ol style="padding-left:18px;line-height:1.8"><li>Open <a href="' + d.searchUrl + '" target="_blank" rel="noopener noreferrer" style="color:#2563eb">' + esc_html(q) + ' on Google Maps ↗</a></li>' +
...
function pickPlace(el) {
  ...
  if (url) {
    ...
    document.getElementById('places-results').innerHTML = '<div style="padding:12px;background:#f0fdf4;border-radius:10px;color:#16a34a;font-size:14px;text-align:center">✓ Review link filled in! Click Continue to proceed.</div>';
  }
}
...
app.get('/templates', requireAuth, (req, res) => {
  ...
  const templates = [
    { cat: '💬 SMS Templates', items: [
      { title: 'After Visit — Friendly', text: `Hi! Thanks for visiting ${bname}. We'd love your feedback — takes 30 seconds: ${rlink}` },
      { title: 'After Visit — Professional', text: `Thank you for choosing ${bname}. Your opinion matters to us! Please share your experience: ${rlink}` },
      { title: 'Follow-Up Reminder', text: `Hi! Just a quick reminder — we'd really appreciate your feedback on ${bname}. It only takes a moment: ${rlink}` },
      { title: 'After Service Completion', text: `Thanks for letting ${bname} take care of you! How did we do? ${rlink}` },
    ]},
    { cat: '✉️ Email Templates', items: [
    ...
    { cat: '🖨️ Print / In-Store', items: [
      { title: 'Receipt Message', text: `Enjoyed your visit? We'd love your feedback!\nScan the QR code or visit: ${rlink}` },
      { title: 'Table Tent / Counter Sign', text: `⭐ Love ${bname}?\nTell us about your experience!\nScan to leave a review →\n${rlink}` },
    ]},
  ];

  let html = `<div style="display:flex;align-items:center;gap:16px;margin-bottom:24px"><a href="/dashboard" style="color:#64748b;text-decoration:none;font-size:14px">← Dashboard</a>
<h1 style="font-size:28px">📋 Ready-Made Templates</h1></div>
...
<button onclick="navigator.clipboard.writeText(document.getElementById('t${tid}').value);this.textContent='✓ Copied!';setTimeout(()=>this.textContent='Copy',2000)" class="btn btn-primary btn-sm">Copy</button></div>
...
<a href="/r/${loc.slug}" target="_blank" rel="noopener noreferrer" class="btn btn-secondary">Preview Live Page ↗</a>
...
function notFoundPage() {
  return `<!DOCTYPE html><html><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1.0">
<title>Page Not Found — ReviewFlow</title>${css()}
<style>body{background:#f8fafc;min-height:100vh;display:flex;flex-direction:column}
.nf{flex:1;display:flex;align-items:center;justify-content:center;text-align:center;padding:40px 24px}</style></head><body>
<nav class="nav"><div class="container nav-inner"><a href="/" class="nav-brand">⭐ ReviewFlow</a>
<div class="nav-links"><a href="/">Home</a><a href="/login">Log In</a></div></div></nav>
<div class="nf"><div>
...
<p style="font-size:20px;color:#64748b;margin-bottom:32px">This page doesn't exist — but your next 5-star review does.</p>
<div style="display:flex;gap:12px;justify-content:center;flex-wrap:wrap">
<a href="/" class="btn btn-primary">Go Home</a>
<a href="/signup" class="btn btn-secondary">Start Collecting Reviews</a>
</div></div></div>
${footer()}</body></html>`;
}
...
app.get('/free-guide', (req, res) => {
  res.send(`<!DOCTYPE html><html lang="en"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1.0">
<title>Free Guide: 10 Ways to Get More 5-Star Google Reviews — ReviewFlow</title>
...
<nav class="nav"><div class="container nav-inner"><a href="/" class="nav-brand">⭐ ReviewFlow</a>
<div class="nav-links"><a href="/blog">Blog</a><a href="/signup" class="btn btn-primary btn-sm">Start Free</a></div></div></nav>
...
<button type="submit" class="btn btn-primary" style="width:100%;padding:14px;font-size:16px">Download Free Guide →</button>
...
app.get('/blog', (req, res) => {
  const posts = BLOG_POSTS.map(p => `
    <article style="margin-bottom:32px;padding-bottom:32px;border-bottom:1px solid #e2e8f0">
      ...
      <h2 style="margin-bottom:8px"><a href="/blog/${p.slug}" style="color:#1e293b;text-decoration:none">${esc(p.title)}</a></h2>
      <p style="color:#64748b;font-size:15px;margin-bottom:12px">${esc(p.description)}</p>
      <a href="/blog/${p.slug}" style="color:#2563eb;font-weight:600;font-size:14px;text-decoration:none">Read more →</a>
    </article>
  `).join('');
  res.send(`<!DOCTYPE html><html lang="en"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1.0">
<title>Blog — ReviewFlow | Google Review Tips for Local Businesses</title>
...
${css()}<style>.blog-wrap{max-width:720px;margin:0 auto;padding:48px 24px}</style></head><body>
<nav class="nav"><div class="container nav-inner"><a href="/" class="nav-brand">⭐ ReviewFlow</a>
<div class="nav-links"><a href="/blog">Blog</a><a href="/#pricing">Pricing</a><a href="/login">Log In</a><a href="/signup" class="btn btn-primary btn-sm">Start Free</a></div></div></nav>
<div class="blog-wrap">
...
app.get('/blog/:slug', (req, res) => {
  ...
  res.send(`<!DOCTYPE html><html lang="en"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1.0">
<title>${esc(post.title)} — ReviewFlow Blog</title>
...
<div class="nav-links"><a href="/blog">Blog</a><a href="/#pricing">Pricing</a><a href="/login">Log In</a><a href="/signup" class="btn btn-primary btn-sm">Start Free</a></div></div></nav>
<div class="blog-wrap">
<a href="/blog" style="color:#64748b;text-decoration:none;font-size:14px">← Back to Blog</a>
...
<a href="/signup" class="btn btn-primary" style="padding:14px 36px;font-size:16px">Start Free — No Credit Card</a>
</div>
</div>
${footer()}</body></html>`);
});
...
app.get('/proposal-templates', requireAuth, (req, res) => {
  const user = getUser(req);
  let html = '<div style="display:flex;align-items:center;gap:16px;margin-bottom:8px"><a href="/dashboard" style="color:#64748b;text-decoration:none;font-size:14px">&larr; Dashboard</a><h1 style="font-size:28px">&#x1f4c4; Proposal Template Library</h1></div>';
...
<title>ReviewFlow Alternatives — Compare Review Management Tools</title>
<meta name="description" content="Compare ReviewFlow vs Birdeye, Podium, BrightLocal, Grade.us, and NiceJob. Find the best review management tool for your local business.">
<meta property="og:title" content="ReviewFlow vs Competitors — Review Management Comparison">
<meta property="og:description" content="Compare ReviewFlow to Birdeye, Podium, BrightLocal and more. Free plan available.">
<meta property="og:url" content="https://alpha.abapture.ai/alternatives">
<link rel="canonical" href="https://alpha.abapture.ai/alternatives">
${css()}
<style>
.comp-table{width:100%;border-collapse:collapse;margin:32px 0}
.comp-table th,.comp-table td{padding:14px 16px;text-align:left;border-bottom:1px solid #e2e8f0;font-size:14px}
.comp-table th{background:#f8fafc;font-weight:600;color:#374151}
.comp-table tr:hover{background:#f8fafc}
.comp-card{border:1px solid #e2e8f0;border-radius:12px;padding:28px;margin-bottom:24px}
.comp-card h3{font-size:20px;margin-bottom:8px}
.price-tag{color:#ef4444;font-weight:700;font-size:18px}
.verdict{background:#f0fdf4;border:1px solid #bbf7d0;border-radius:8px;padding:12px 16px;margin-top:16px;font-size:14px}
</style></head><body>
<nav class="nav"><div class="container nav-inner"><a href="/" class="nav-brand">⭐ ReviewFlow</a>
<div class="nav-links">${user?'<a href="/dashboard">Dashboard</a><a href="/logout">Log Out</a>':'<a href="/login">Log In</a><a href="/signup" class="btn btn-primary btn-sm">Sign Up</a>'}</div></div></nav>
<section style="padding:60px 0;background:linear-gradient(135deg,#eff6ff,#f8fafc)">
<div class="container" style="max-width:800px;text-align:center">
<h1 style="font-size:40px;font-weight:800;color:#1e293b;margin-bottom:16px">ReviewFlow vs. The Competition</h1>
<p style="font-size:18px;color:#64748b;max-width:600px;margin:0 auto">See how ReviewFlow compares to expensive review management platforms. Spoiler: we offer a free plan.</p>
</div></section>

<section style="padding:60px 0"><div class="container" style="max-width:900px">
<h2 style="font-size:28px;font-weight:700;margin-bottom:24px;text-align:center">Quick Comparison</h2>
<div style="overflow-x:auto">
<table class="comp-table">
<thead><tr><th>Feature</th><th style="color:#2563eb">ReviewFlow</th><th>Birdeye</th><th>Podium</th><th>BrightLocal</th><th>Grade.us</th></tr></thead>
<tbody>
<tr><td><strong>Starting Price</strong></td><td style="color:#16a34a;font-weight:700">Free</td><td class="price-tag">$299/mo</td><td class="price-tag">$399/mo</td><td>$39/mo</td><td>$110/mo</td></tr>
<tr><td>Smart Review Gating</td><td style="color:#16a34a">Yes</td><td style="color:#16a34a">Yes</td><td style="color:#16a34a">Yes</td><td style="color:#ef4444">No</td><td style="color:#16a34a">Yes</td></tr>
<tr><td>QR Code Generator</td><td style="color:#16a34a">Yes</td><td style="color:#16a34a">Yes</td><td style="color:#16a34a">Yes</td><td style="color:#ef4444">No</td><td style="color:#16a34a">Yes</td></tr>
<tr><td>Custom Branding</td><td style="color:#16a34a">Yes</td><td style="color:#16a34a">Yes</td><td style="color:#16a34a">Yes</td><td style="color:#ef4444">No</td><td style="color:#16a34a">Yes</td></tr>
<tr><td>Free Tier</td><td style="color:#16a34a"><strong>Yes - full features</strong></td><td style="color:#ef4444">No</td><td style="color:#ef4444">No</td><td style="color:#ef4444">No</td><td style="color:#ef4444">No</td></tr>
<tr><td>Setup Time</td><td><strong>2 minutes</strong></td><td>30+ min</td><td>Sales call</td><td>15 min</td><td>10 min</td></tr>
<tr><td>Contract Required</td><td>No</td><td>Annual</td><td>Annual</td><td>No</td><td>No</td></tr>
</tbody></table></div>

<div style="margin-top:48px">
<div class="comp-card">
<h3>ReviewFlow vs Birdeye</h3>
<p class="price-tag">Birdeye: $299/mo+ (annual contract)</p>
<p style="margin-top:8px;color:#64748b">Birdeye is a full-suite reputation platform. Great for enterprises — overkill and overpriced for most local businesses.</p>
<div class="verdict">If you just need more Google reviews, ReviewFlow does the job at 1/10th the cost (or free).</div>
</div>

<div class="comp-card">
<h3>ReviewFlow vs Podium</h3>
<p class="price-tag">Podium: $399/mo+ (annual contract)</p>
<p style="margin-top:8px;color:#64748b">Podium bundles reviews with payments and messaging. Powerful but complex and expensive. Most small businesses only use the review features.</p>
<div class="verdict">Unless you need payment processing and webchat, ReviewFlow delivers the core value for free.</div>
</div>

<div class="comp-card">
<h3>ReviewFlow vs BrightLocal</h3>
<p class="price-tag">BrightLocal: $39/mo</p>
<p style="margin-top:8px;color:#64748b">BrightLocal is primarily an SEO and citation tool. No review gating, no QR codes, no branded review pages.</p>
<div class="verdict">Use BrightLocal for SEO audits, ReviewFlow for review collection. Or just use ReviewFlow free and save $39/mo.</div>
</div>

<div class="comp-card">
<h3>ReviewFlow vs Grade.us</h3>
<p class="price-tag">Grade.us: $110/mo per seat</p>
<p style="margin-top:8px;color:#64748b">Grade.us is built for agencies managing multiple clients. Per-seat pricing adds up fast.</p>
<div class="verdict">Grade.us makes sense for agencies. For individual businesses, ReviewFlow is simpler and free.</div>
</div>

<div class="comp-card">
<h3>ReviewFlow vs NiceJob</h3>
<p class="price-tag">NiceJob: $75/mo</p>
<p style="margin-top:8px;color:#64748b">NiceJob automates review requests via email/SMS. Solid product, but $75/mo is steep for a small business starting out.</p>
<div class="verdict">NiceJob has more automation, but ReviewFlow free tier gives you the essentials without the monthly bill.</div>
</div>
</div>

<div style="text-align:center;margin-top:48px;padding:40px;background:linear-gradient(135deg,#eff6ff,#dbeafe);border-radius:16px">
<h2 style="font-size:28px;font-weight:700;margin-bottom:12px">Ready to try ReviewFlow?</h2>
<p style="color:#64748b;margin-bottom:24px;font-size:16px">Free plan includes everything you need. Set up in 2 minutes.</p>
<a href="/signup" class="btn btn-primary" style="padding:16px 40px;font-size:17px">Get Started Free</a>
</div>
</div></section>
${footer()}
</body></html>`;
