...
# Create the patch using Node
node -e "
const fs = require('fs');
let code = fs.readFileSync('server.js', 'utf8');

// 1. Add response_templates table creation after notifications table
const tableInsert = \`
...
// 2. Add nav link for Response Templates in dashLayout
code = code.replace(
  '<a href=\"/review-analytics\">📊 Analytics</a>',
  '<a href=\"/review-analytics\">📊 Analytics</a><a href=\"/response-templates\">📝 Templates</a>'
);

// 3. Add response templates routes before the 404 handler
const routesCode = \`
...
app.get('/response-templates', requireAuth, (req, res) => {
  ...
  templates.forEach(t => {
    ...
    html += '<div class="card" style="position:relative;border-left:4px solid ' + color + ';transition:transform 0.15s;cursor:default" onmouseenter="this.style.transform=\\\\'translateY(-2px)\\\\'" onmouseleave="this.style.transform=\\\\'none\\\\'">';
    ...
    html += '<button onclick="copyTpl(this, \\\\'' + esc(t.id) + '\\\\')" class="btn btn-primary btn-sm" style="font-size:13px">📋 Copy</button>';
    html += '<button onclick="expandTpl(\\\\'' + esc(t.id) + '\\\\')" class="btn btn-sm" style="font-size:13px;background:#f1f5f9;border:1px solid #e2e8f0;color:#334155">👁 Preview</button>';
    html += '</div></div>';
  });
  html += '</div>';

  // New template modal
  html += '<div id="newTplModal" style="display:none;position:fixed;inset:0;background:rgba(0,0,0,0.5);z-index:1000;align-items:center;justify-content:center;padding:24px">';
  html += '<div style="background:#fff;border-radius:16px;padding:32px;max-width:560px;width:100%;max-height:90vh;overflow-y:auto">';
  html += '<div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:20px"><h2 style="font-size:22px;margin:0">Create Custom Template</h2><button onclick="document.getElementById(\\\\'newTplModal\\\\').style.display=\\\\'none\\\\'" style="background:none;border:none;font-size:24px;cursor:pointer;color:#64748b">&times;</button></div>';
  html += '<form id="newTplForm" onsubmit="saveTpl(event)">';
  ...
  html += '<div class="form-group"><label>Template Name</label><input name="title" required placeholder="e.g. My Thank You Response" style="width:100%;padding:10px;border:1px solid #e2e8f0;border-radius:8px"></div>';
  html += '<div class="form-group"><label>Response Text</label><textarea name="body" required rows="6" placeholder="Write your template response..." style="width:100%;padding:10px;border:1px solid #e2e8f0;border-radius:8px;resize:vertical"></textarea></div>';
  html += '<button type="submit" class="btn btn-primary" style="width:100%;padding:12px">Save Template</button>';
  html += '</form></div></div>';

  // Preview modal
  html += '<div id="previewModal" style="display:none;position:fixed;inset:0;background:rgba(0,0,0,0.5);z-index:1000;align-items:center;justify-content:center;padding:24px">';
  html += '<div style="background:#fff;border-radius:16px;padding:32px;max-width:560px;width:100%;max-height:90vh;overflow-y:auto">';
  html += '<div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:16px"><h2 id="previewTitle" style="font-size:20px;margin:0"></h2><button onclick="document.getElementById(\\\\'previewModal\\\\').style.display=\\\\'none\\\\'" style="background:none;border:none;font-size:24px;cursor:pointer;color:#64748b">&times;</button></div>';
  html += '<div id="previewBody" style="white-space:pre-wrap;line-height:1.7;color:#334155;font-size:15px;background:#f8fafc;padding:20px;border-radius:12px"></div>';
  html += '<button onclick="navigator.clipboard.writeText(document.getElementById(\\\\'previewBody\\\\').textContent);this.textContent=\\\\'✓ Copied!\\\\';setTimeout(()=>this.textContent=\\\\'📋 Copy to Clipboard\\\\',2000)" class="btn btn-primary" style="width:100%;margin-top:16px;padding:12px">📋 Copy to Clipboard</button>';
  html += '</div></div>';

  // JS
  html += '<script>';
  html += 'const tplData = ' + JSON.stringify(templates.map(t => ({ id: t.id, title: t.title, body: t.body }))) + ';';
  html += 'function copyTpl(btn, id) { const t = tplData.find(x=>x.id===id); if(t){navigator.clipboard.writeText(t.body);btn.textContent="✓ Copied!";setTimeout(()=>btn.textContent="📋 Copy",2000);} }';
  html += 'function expandTpl(id) { const t = tplData.find(x=>x.id===id); if(t){document.getElementById("previewTitle").textContent=t.title;document.getElementById("previewBody").textContent=t.body;document.getElementById("previewModal").style.display="flex";} }';
  html += 'async function saveTpl(e) { e.preventDefault(); const f=new FormData(e.target); const r=await fetch("/api/response-templates",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({category:f.get("category"),title:f.get("title"),body:f.get("body")})}); const d=await r.json(); if(d.ok) location.reload(); else alert(d.error||"Error saving"); }';
  html += 'async function deleteTpl(id) { if(!confirm("Delete this template?")) return; const r=await fetch("/api/response-templates/"+id,{method:"DELETE"}); const d=await r.json(); if(d.ok) location.reload(); else alert(d.error||"Error"); }';
  html += '</script>';
...
// API: Get templates for picker (used in review detail)
...
echo "Patch applied, restarting..."
pm2 restart reviewflow 2>/dev/null || (pkill -f 'node server.js' 2>/dev/null; sleep 1; cd /opt/reviewflow && nohup node server.js > /tmp/reviewflow.log 2>&1 &)
sleep 2
